...
class Photothermal_Image:
		...
		# memory allocation 
                ...
                if int(nplanes) > 0:
                        f.write(" 1 = NPLANES = number of scattering planes\n")
                        f.write(" 0 0 0 1 = phi, theta_min, theta_max (deg) for plane %r\n")
                f.close()
        ...
        def field_MM(self, filename,skip):
            ...
            theta = theta_fml
            phi = phi_fml
            Ex = E_r*np.cos(theta) - E_theta*np.sin(theta)
            Ey = E_r*np.sin(theta)*np.cos(phi) + E_theta*np.cos(theta)*np.cos(phi) - E_phi*np.sin(phi)
            Ez = E_r*np.sin(theta)*np.sin(phi) + E_theta*np.cos(theta)*np.sin(phi) + E_phi*np.cos(phi)
            ...
            return np.array([Ex, Ey, Ez]), theta, phi, theta_fml, phi_fml
        ...
        def calculated_IPT(self):
                ...
                for valy in range(-self.image_width,self.image_width+self.ss,self.ss):
                        for valz in range(-self.image_width,self.image_width+self.ss,self.ss):
                                ...
                                E_H, theta, phi, theta_fml, phi_fml = self.field_MM(filename=str(base_file+str('H')),skip=base+ndiff) 
                                E_R, _, _,_,_ = self.field_MM(filename=str(base_file+str('R')),skip=base)
                                ...
                                # Total 
                                print(c*self.n_R/(8*np.pi)*( hot_minus_room ))

                                IPT_TOT_HMR[yi, zi] = c*self.n_R/(8*np.pi)*( hot_minus_room )
                                IPT_TOT_INT[yi, zi] = c*self.n_R/(8*np.pi)*( interf_term )
        ...
        def collect_fml(self):
                ...
                file = open(str('pt_signal_no_int.txt'),'w')
